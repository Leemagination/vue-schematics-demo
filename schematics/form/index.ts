...
function updateApp() {
  return (tree: Tree) => {
    ...
    const matchStr = appFile?.content.toString().match(/id="formModalContainer"(?:.|\n)*?(?=<\/div>)/)
    ...
    const matchStr2 = appFile?.content.toString().match(/id="formContainer"(?:.|\n)*?(?=<\/template>)/)
    ...
    const matchStr3 = appFile?.content.toString().match(/@Component\({(?:.|\n)*?(?=})/)
